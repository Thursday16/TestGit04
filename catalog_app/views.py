...
def player_list(request):
    #qs = PlayVideo.objects.all()
    list = [{'name': x.name, 'lastname': x.lastname, 'shirt': str(x.shirt),
        'username': x.username, 'email': x.email, 'phone': str(x.phone),
         }
            for x in Player.objects.filter(void=0).order_by('-created_time')]
    #qs_json = serializers.serialize('json', list)
    qs_json = json.dumps(list)
    return HttpResponse(qs_json, content_type='application/json')
...
def player_detail(request):
    ...
    list = [{
        ...
        'name': x.name, 'lastname': x.lastname, 'shirt': str(x.shirt),
        'username': x.username, 'email': x.email, 'phone': str(x.phone),
        'updated_time': x.updated_time.strftime('%Y-%m-%d %H:%M:%S'),
        'created_time': x.created_time.strftime('%Y-%m-%d %H:%M:%S'),
    }
        for x in Player.objects.filter(name=name).filter(void=0).order_by('-created_time')]
